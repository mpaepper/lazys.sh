...
if [ "$1" == "--help" ]; then
  echo "Call with cp 'from' 'to' to copy from server to here. Call with cpto 'from' 'to' to to copy from here to server."
  echo "Call 'lazys.sh create \$connectionKey \$name@\$server \$port' to create a new entry for the \$connectionKey which you can use afterwards to connect. The \$port is optional."
  exit 1
elif [ "$1" == "create" ]; then
  key=$2
  spli=(${3/@/ })
